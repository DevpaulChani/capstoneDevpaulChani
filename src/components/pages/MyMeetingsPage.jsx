...
export default function MyMeetingsPage({ currentUser, meetingRequests, users, availableTimes, onLogout, onCancelMeeting }) {
  ...
  const getMeetingDetails = (meeting) => {
    ...
    const otherUserId = meeting.requesterId === currentUser.id ? meeting.requesteeId : meeting.requesterId
    const otherUser = users.find(u => u.id === otherUserId)
    const role = meeting.requesterId === currentUser.id ? 'requested' : 'received'
      ...
      {myMeetings.length === 0 ? (
        ...
        <div>
          {myMeetings.map(meeting => {
            ...
            return (
              <div key={meeting.id} style={{ border: '1px solid black', padding: '10px', marginBottom: '10px' }}>
                <p>You have a meeting at {details.time} with {details.otherUser} ({details.role})</p>
                <button onClick={() => handleCancelMeeting(meeting.id)}>Cancel Meeting</button>
              </div>
            )
          })}
        </div>
