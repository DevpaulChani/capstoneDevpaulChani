...
export default function App() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [usersData, timesData, requestsData, statusesData] = await Promise.all([
          api.getUsers(),
  ...
  const handleAcceptRequest = async (requestId) => {
    try {
      ...
      if (updatedRequest) {
        setMeetingRequests(
          meetingRequests.map(req => req.id === requestId ? updatedRequest : req)
        )
      }
    } catch (error) {
  ...
  const handleCancelMeeting = async (meetingId) => {
    try {
      ...
      if (success) {
        setMeetingRequests(meetingRequests.filter(req => req.id !== meetingId))
      }
    } catch (error) {
